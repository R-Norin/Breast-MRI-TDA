"""
Stage 2 — Final runs — Fusion models (image + TDA)
...
Usage:
    python stage2_fusion.py --model r3d18 --seed 42
    python stage2_fusion.py --model mc318 --seed 52
    python stage2_fusion.py --model r2plus1d --seed 32
...
# PATHS
# ============================================================
DATA_ROOT = Path("breast_mri/dataset")
TDA_ROOT  = Path("breast_mri/tda")
...
def load_best_params(model_name):
    path = STAGE1_OUT / f"{model_name}_fusion" / "best_params.json"
    if not path.exists():
...
def main(args):
    out_dir  = STAGE2_OUT / f"{args.model}_fusion"
    ...
    params = load_best_params(args.model)
    ...
    fastmri_ds  = FusionDataset(FASTMRI_IMG_ROOT,  FASTMRI_TDA_CSV,  scaler=train_ds.scaler)
    breastdm_ds = FusionDataset(BREASTDM_IMG_ROOT, BREASTDM_TDA_CSV, scaler=train_ds.scaler)
    ...
    best_epoch   = -1
    wait         = 0
    best_ckpt    = out_dir / f"best_{args.model}_seed{args.seed}.pt"
    ...
    # -------------------------
    _, y_fm,  p_fm  = predict(model, fastmri_loader,  DEVICE, fusion=True)
    _, y_bdm, p_bdm = predict(model, breastdm_loader, DEVICE, fusion=True)
