"""
Stage 2 — Final runs — Swin UNETR Fusion (image + TDA)
...
def load_best_params():
    path = STAGE1_OUT / "swin_fusion" / "best_params.json"
    if not path.exists():
...
def main(args):
    out_dir  = STAGE2_OUT / "swin_fusion"
    ...
    print(f"Swin UNETR Fusion | Seed: {args.seed}")
    ...
    best_epoch   = -1
    wait         = 0
    best_ckpt    = out_dir / f"best_swin_fusion_seed{args.seed}.pt"
