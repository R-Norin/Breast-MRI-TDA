"""
Unified TDA-fusion training script for Breast MRI models.
...
Features:
    - hyperparameter search over batch size, learning rate, weight decay, and dropouts
...
Expected image dataset structure:
    DATA_ROOT/
        train/
            benign/
            malignant/
...
def get_args():
    ...
    p.add_argument("--tda_train_csv", required=True)
    p.add_argument("--tda_val_csv", required=True)
    p.add_argument("--tda_test_csv", required=True)
...
def make_combinations(
    ...
    combos = []
    for bs, lr, wd, di, dt, df in itertools.product(
        batch_sizes,
        learning_rates,
        weight_decays,
        dropout_image,
...
def build_model(model_name, combo, tda_dim, swin_pretrained_weights):
...
def run_one_seed(args, model_name, combo, seed, combo_idx):
    ...
    device = get_device()
    print("\n" + "=" * 100)
    print(f"FUSION MODEL={model_name} | COMBO={combo_idx} | SEED={seed}")
    print(combo)
    print("Device:", device)
    print("=" * 100)

    run_name = (
        f"{model_name}_fusion_combo{combo_idx}_seed{seed}"
    ...
    val_ids, y_val, p_val = predict(model, val_loader, device, fusion=True)
    test_ids, y_test, p_test = predict(model, test_loader, device, fusion=True)
...
def main():
    ...
    combos = make_combinations(
        ...
        dropout_image=parse_float_list(args.dropout_image),
        dropout_tda=parse_float_list(args.dropout_tda),
        dropout_fusion=parse_float_list(args.dropout_fusion),
    )
    ...
    print("\n" + "=" * 120)
    print("ALL FUSION RESULTS SORTED")
    print("=" * 120)
